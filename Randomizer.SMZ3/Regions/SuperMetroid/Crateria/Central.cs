        ...
        public Central(World world, Config config) : base(world, config) {
            Locations = new List<Location> {
                new Location(this, 0, 0x8F81CC, LocationType.Visible, "Power Bomb (Crateria surface)", Logic switch {
                    _ => new Requirement(items => (config.SMKeysanity ? items.CardCrateriaL1 : items.CanUsePowerBombs()) && (items.SpeedBooster || items.CanFly()))
                }),
                ...
                new Location(this, 7, 0x8F8404, LocationType.Chozo, "Bombs", Logic switch {
                    Normal => items => (config.SMKeysanity ? items.CardCrateriaBoss : items.CanOpenRedDoors()) && items.CanPassBombPassages(),
                    _ => new Requirement(items => (config.SMKeysanity ? items.CardCrateriaBoss : items.CanOpenRedDoors()) && items.Morph)
                })
            };
        }
