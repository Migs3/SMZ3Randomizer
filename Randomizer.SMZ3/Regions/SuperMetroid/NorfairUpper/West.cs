        ...
        public West(World world, Config config) : base(world, config) {
            Locations = new List<Location> {
                ...
                new Location(this, 50, 0x8F8B24, LocationType.Chozo, "Ice Beam", Logic switch {
                    Normal => items => (config.SMKeysanity ? items.CardNorfairL1 : items.Super) && items.CanPassBombPassages() && items.Varia && items.SpeedBooster,
                    _ => new Requirement(items => (config.SMKeysanity ? items.CardNorfairL1 : items.Super) && items.Morph && (items.Varia || items.HasEnergyReserves(3)))
                }),
                new Location(this, 51, 0x8F8B46, LocationType.Hidden, "Missile (below Ice Beam)", Logic switch {
                    Normal => items => (config.SMKeysanity ? items.CardNorfairL1 : items.Super) && items.CanUsePowerBombs() && items.Varia && items.SpeedBooster,
                    _ => new Requirement(items =>
                        (config.SMKeysanity ? items.CardNorfairL1 : items.Super) && items.CanUsePowerBombs() && (items.Varia || items.HasEnergyReserves(3)) ||
                        (items.Missile || items.Super || items.Wave /* Blue Gate */) && items.Varia && items.SpeedBooster &&
                            /* Access to Croc's room to get spark */
                            (config.SMKeysanity ? items.CardNorfairBoss : items.Super) && items.CardNorfairL1)
                }),
